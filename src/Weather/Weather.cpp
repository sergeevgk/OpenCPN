...
#include "Weather/weather_utils.h"
#include "Weather/weather_render_utilities.h"
...
const std::string TimeMeasureFileName = "plugins\\weather_pi\\data\\time.txt";
...
Weather::Weather()
{
	...
	get_all_weather_date_data("plugins\\weather_pi\\data\\weather_data.csv");
	create_data_grid();
...
WeatherUtils::RouteCheckData Weather::analyseRouteCheck(ChartCanvas *cc, ocpnDC& dc, ViewPort &VP, const LLBBox &box, Route *route, double start_time_shift, bool build_rescue_root, bool draw_cone_lines) 
{
	...
	for (node = node->GetNext(); node; node = node->GetNext()) {
		...
		while (((std::round(prev_grid_lat * 10)) != std::round(lat_finish * 10)) || (std::round(prev_grid_lon * 10) != std::round(lon_finish * 10))) {
			...
			check_depth_in_cone(s57ch, cc, dc, VP, box, start, end, angle, routeCheckData.conflicts, sum_time);
...
void Weather::check_depth_in_cone(s57chart* chart, ChartCanvas *cc, ocpnDC& dc, ViewPort &VP,
	const LLBBox &box, wxPoint2DDouble start, wxPoint2DDouble end, double angle, std::vector<WeatherUtils::ConflictData> &conflicts, double shift_time){
...
bool Weather::print_objects_values_to_file(ListOfObjRazRules* list, s57chart* chart) {
	...
	std::ofstream myfile;
	myfile.open("plugins\\weather_pi\\data\\debug\\output.txt", std::ofstream::app);
...
void Weather::draw_gradient(ChartCanvas *cc, ocpnDC& dc, ViewPort &VP, const LLBBox &box) {
	...
	//SaveKeyValueToFile(TimeMeasureFileName, "count", count);
}
...
bool Weather::download_weather_from_esimo() {
	...
	curl = curl_easy_init();
	if (curl) {
		fp = fopen("plugins\\weather_pi\\data\\weather_data.csv", "wb");
		//fp = fopen("C:\\Users\\Admin\\Desktop\\TEST\\lena_weather_data.csv", "wb");
